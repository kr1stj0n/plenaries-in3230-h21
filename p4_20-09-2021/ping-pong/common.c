		    ...
		    uint8_t *packet)
{
	...
	msgvec[2].iov_len  = strlen(packet) + 1;


	/* Allocate a zeroed-out message info struct */
	msg = (struct msghdr *)calloc(1, sizeof(struct msghdr));

	/* Make sure to send the msg to the right destination MAC address */
	memcpy(ifs->addr[0].sll_addr, frame_hdr.dst_addr, 6);
	...
	printf("Sending a MIP packet with content '%s' to node %u with MAC address: \n",
...
int handle_mip_packet(struct ifs_data *ifs)
{
	...
	printf("<info>: We got a MIP packet with content '%s' from node %d with MAC address: ",
